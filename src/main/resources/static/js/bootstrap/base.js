...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            init: function (options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    clickToSelect: true,                                 // 点击行选中checkbox
                    ...
                    onPostBody: options.onPostBody,                     // 当行数据被改变时触发处理函数
                    onLoadSuccess: options.onLoadSuccess,               // 当所有数据被加载时触发处理函数
                    rowStyle: options.rowStyle,                        // 自定义行样式
                  //  cellStyle: options.rowStyle                        // 自定义行样式
            ...
            // 获取字典Label值
            selectDictName:function(datas, value){
                var actions = [];
                $.each(datas, function (index, dict) {
                    if (dict.dictValue == value) {
        ...
        modal: {
            ...
            //保存确认刷新
            saveLoad:function (content, type) {
                layer.alert(content, {
                    icon: $.modal.icon(type),
                    title: "系统提示",
                    btn: ['确认'],
        ...
        operate: {
            ...
            // 保存然后重新加载选项卡
            saveReloadTab: function (url, data) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
                    ...
                    success: function (result) {
                        $.modal.saveLoad(result.msg,result.code);
                    }
                };
                $.ajax(config)
            },
            ...
            successTabCallback: function (result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var $contentWindow = $('.Memento_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
            ...
            savePage:function (url,data) {
                var config = {
                    ...
                    success: function(result) {
                        $.modal.closeLoading();
                        $.modal.saveLoad(result.msg,modal_status.SUCCESS);
...
flow_status = {
    ...
    FIRST_REVIEW_FAIL: "初审未通过;",
    MIDDLE_REVIEW_PASS : "二审通过;",
    MIDDLE_REVIEW_FAIL : "二审未通过;",
    SCHEME_HANDLE : "上级介入处理;",
    MANAGER_HANDLE : "部门直接处理;",
    LAST_REVIEW_PASS : "终审通过",
    LAST_REVIEW_FAIL : "终审未通过"
};